setwd('/users/hslee/Pink1/cancer')
library(ggplot2)
library(reshape)
library(ggpubr)
library(rstatix)
...
all_box_plot  +
  stat_pvalue_manual(
    ...
    vjust = -0.5,
    bracket.nudge.y = 2
  ) + geom_jitter(alpha = 0.35, color = "#2e4057" , position=position_jitter(0.35)) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1)))
...
stat.test_all_1
all_box_plot_1 + geom_jitter(alpha = 0.4, color = "#2e4057" , position=position_jitter(0.35)) + stat_pvalue_manual(stat.test_all_1, label = "p.adj.signif", tip.length = 0.01)
...
stat.test_all_2
all_box_plot_2 + geom_jitter(alpha = 0.4, color = "#2e4057" , position=position_jitter(0.35)) + stat_pvalue_manual(stat.test_all_2, label = "p.adj.signif", tip.length = 0.01, y.position = 40 )
