...
# exp file
normExp <- read.csv("pink1_norm.csv", row.names = 1)

#DESeq
statsDE <- read.csv("deseq_results.csv")
statsDE <- na.omit(statsDE)
statsDE <- statsDE[order(statsDE$padj),]
write.csv(statsDE,"ttest1.csv")

statsDE <- read.csv("ttest1.csv")
...
GOI <- read.table('HIF1a_GOI.txt', header = T, sep = '\t')
GOI1 <- merge(GOI, statsDE, by = 1 )
